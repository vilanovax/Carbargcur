...
const TAX_BRACKETS_1403: TaxBracket[] = [
  { from: 0, to: 10_000_000, rate: 0, label: "تا ۱۰ میلیون تومان - معاف" },
  { from: 10_000_000, to: 20_000_000, rate: 0.10, label: "۱۰ تا ۲۰ میلیون - ۱۰٪" },
  { from: 20_000_000, to: 40_000_000, rate: 0.15, label: "۲۰ تا ۴۰ میلیون - ۱۵٪" },
  { from: 40_000_000, to: null, rate: 0.20, label: "بالای ۴۰ میلیون - ۲۰٪" },
];
...
export default function TaxCalculatorPage() {
  ...
  const calculateTax = () => {
    ...
    // محاسبه پلکانی مالیات
    if (totalIncome > 40_000_000) {
      // پله چهارم
      const amount4 = totalIncome - 40_000_000;
      const tax4 = amount4 * 0.20;
      tax += tax4;
      brackets.push({ bracket: "بالای ۴۰ میلیون (۲۰٪)", amount: amount4, tax: tax4 });

      // پله سوم
      const amount3 = 40_000_000 - 20_000_000;
      const tax3 = amount3 * 0.15;
      tax += tax3;
      brackets.push({ bracket: "۲۰ تا ۴۰ میلیون (۱۵٪)", amount: amount3, tax: tax3 });

      // پله دوم
      const amount2 = 20_000_000 - 10_000_000;
      const tax2 = amount2 * 0.10;
      tax += tax2;
      brackets.push({ bracket: "۱۰ تا ۲۰ میلیون (۱۰٪)", amount: amount2, tax: tax2 });

      // پله اول
      brackets.push({ bracket: "تا ۱۰ میلیون (معاف)", amount: 10_000_000, tax: 0 });
    } else if (totalIncome > 20_000_000) {
      // پله سوم
      const amount3 = totalIncome - 20_000_000;
      const tax3 = amount3 * 0.15;
      tax += tax3;
      brackets.push({ bracket: "۲۰ تا ۴۰ میلیون (۱۵٪)", amount: amount3, tax: tax3 });
      ...
      // پله دوم
      const amount2 = totalIncome - 10_000_000;
      const tax2 = amount2 * 0.10;
      tax += tax2;
      brackets.push({ bracket: "۱۰ تا ۲۰ میلیون (۱۰٪)", amount: amount2, tax: tax2 });
      ...
      // فقط پله اول - معاف
      brackets.push({ bracket: "تا ۱۰ میلیون (معاف)", amount: totalIncome, tax: 0 });
    }
